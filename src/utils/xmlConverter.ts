...
/**
 * Parse XML string and convert to flat table data format
 * Handles the nested Batch > Contract > ContractData structure
 */
export const parseXMLToTableData = (xmlString: string): TableData[] => {
  try {
...
/**
 * Flatten a single Batch/Contract element into table row data
 */
const flattenBatchContractData = (batchIdentifier: string, contractElement: Element): TableData => {
  const row: TableData = {};
...
/**
 * Convert flat table data to properly nested XML format with schema
 */
export const convertToXML = (data: TableData[]): string => {
...
`;
  ...
  // Add Batch Identifier (use the first record's BatchIdentifier or empty)
